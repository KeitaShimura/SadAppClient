...
describe('RegisterFormコンポーネント', () => {
    it('正しいデータでフォームが送信された場合、アカウント登録が成功すること', async () => {
        ...
        // フォームにデータを入力
        fireEvent.change(getByPlaceholderText('名前'), { target: { value: 'テストユーザー' } });
        fireEvent.change(getByPlaceholderText('メールアドレス'), { target: { value: 'test@example.com' } });
        fireEvent.change(getByPlaceholderText('パスワード'), { target: { value: 'password123' } });
        fireEvent.change(getByPlaceholderText('パスワード確認'), { target: { value: 'password123' } });
    ...
    it('フォームが正しくバリデーションされること', async () => {
        ...
        expect(queryByText('全ての項目を入力してください。')).not.toBeInTheDocument();

        // フォームに不正なデータを入力
        fireEvent.change(getByPlaceholderText('名前'), { target: { value: '' } });
        fireEvent.change(getByPlaceholderText('メールアドレス'), { target: { value: 'invalid-email' } });
        fireEvent.change(getByPlaceholderText('パスワード'), { target: { value: 'short' } });
        fireEvent.change(getByPlaceholderText('パスワード確認'), { target: { value: 'passwords-dont-match' } });
